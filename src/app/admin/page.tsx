  ...
  async function signInWithGoogle() {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      ...
      options: {
        ...
        // Always show Google's account picker rather than silently reusing
        // whatever Google account happens to already be signed into this
        // browser (e.g. a merchant's own Google session) - this is what
        // caused signing in as danny@feedme.gg unexpectedly.
        queryParams: { prompt: 'select_account' },
      }
    })
    if (error) setAuthError('Could not sign in with Google')
  }
  ...
  useEffect(() => {
    async function checkExistingSession() {
      ...
      if (user.id === ADMIN_USER_ID) {
        ...
        // Signed in (e.g. via Google) but this account's ID doesn't match
        // the authorised admin ID yet - show it directly so it can be
        // copied and sent over to update ADMIN_USER_ID, rather than having
        // to go digging through Supabase's dashboard to find it.
        setAuthError(`Signed in as ${user.email}, but this account isn't authorised as admin yet. Its ID is: ${user.id}`)
      ...
      <div style={{ background: 'var(--bg)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
        <div style={{ width: '100%', maxWidth: '380px' }}>
          ...
          <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '16px', padding: '28px' }}>
            ...
            {wrongAccount && (
              <button onClick={signOutAndRetry} style={{ width: '100%', padding: '10px', background: 'rgba(255,255,255,0.06)', border: '1px solid var(--border)', color: 'var(--text)', borderRadius: '8px', fontSize: '13px', fontWeight: 600, cursor: 'pointer', marginBottom: '14px' }}>
                Sign out and try a different account
              </button>
            )}
            <button onClick={signInWithGoogle} style={{ width: '100%', padding: '12px', background: 'white', color: '#1f2937', border: '1px solid var(--border)', borderRadius: '10px', fontWeight: 600, fontSize: '14px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}>
