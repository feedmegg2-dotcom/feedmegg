  ...
  try {
    ...
    if (orderType === 'delivery' && deliveryPostcode) {
      ...
      const zone = zones?.find(z => 
        z.postcodes.some((pc: string) => deliveryPostcode.toUpperCase().startsWith(pc))
      )

      if (!zone) {
        return NextResponse.json({ error: 'Sorry, we don\'t deliver to that postcode' }, { status: 400 })
      }

      deliveryFee = zone.fee
      if (zone.free_delivery_over && subtotal >= zone.free_delivery_over) {
        deliveryFee = 0
      }
    }
